...
assert.equal(source.includes("// @version      0.4.8"), true);
assert.equal(source.includes("getKeyValue:"), true);
assert.equal(source.includes("getKeys: \"query GetApiKeys($first:Int,$after:Cursor,$orderBy:APIKeyOrder,$where:APIKeyWhereInput){apiKeys(first:$first,after:$after,orderBy:$orderBy,where:$where){edges{node{id name key status user{id}}cursor}pageInfo{hasNextPage endCursor}totalCount}}\""), true);
