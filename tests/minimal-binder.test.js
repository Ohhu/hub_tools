...
assert.equal(source.includes('data-action="toggle-key-status"'), true);
assert.equal(source.includes('data-action="rotate-key"'), true);
assert.equal(source.includes('data-action="archive-key"'), true);
assert.equal(source.includes("hkb-key-option-row"), true);
...
assert.equal(source.includes(".hkb-card{width:min(480px,100%);height:392px"), true);
assert.equal(source.includes("[data-view-panel]{height:100%;display:grid;grid-template-rows:auto 1fr auto}"), true);
assert.equal(source.includes("button:not(.hkb-icon-btn):not(.hkb-key-toggle):not(.hkb-key-act){box-sizing:border-box;height:32px;min-height:32px;line-height:18px"), true);
assert.equal(source.includes("hkb-edit-body{display:grid;grid-template-rows:auto auto auto minmax(0,1fr)"), true);
