...
const queries = {
  ...
  getKey: "query GetApiKey($id:ID!){node(id:$id){... on APIKey{id name status profiles{activeProfile profiles{name modelMappings{from to} channelIDs channelTags channelTagsMatchMode modelIDs loadBalanceStrategy channelBindingMode routingPolicy{channelWeights{channelID weight} providerWeights{provider weight} includeProviders excludeProviders allowChannelAPIKeys{channelID keyHashes} excludeChannelAPIKeys{channelID keyHashes} fallbackChannelIDs excludeChannelIDs} dynamicChannelStrategy{mode selectionPolicy maxChannels minChannels maxPriceMultiplier maxLatencyMs minSuccessRate minCacheRate minDetectionScore onlyOfficial includeTags excludeTags excludeChannelIDs fallbackChannelIDs} quota{requests totalTokens cost period{type pastDuration{value unit} calendarDuration{unit}}}}}}}}",
